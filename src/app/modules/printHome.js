...
export const listarContactos = async (idUserLogged, contactsContainer) => {
  try {
    ...
    // Renderizar los contactos iniciales
    renderizarContactos(contactos, contactsContainer);

    // Función para filtrar y renderizar contactos
    const filtrarYRenderizarContactos = () => {
      const textoBusqueda = inputSearchContact.value.toLowerCase(); // Convertir el texto de búsqueda a minúsculas
      const contactosFiltrados = contactos.filter(contacto => contacto.nombre.toLowerCase().includes(textoBusqueda));
      renderizarContactos(contactosFiltrados, contactsContainer);
    };

    // Agregar un evento de escucha al campo de entrada
    const inputSearchContact = document.getElementById('inputSearchContact');
    inputSearchContact.addEventListener('input', filtrarYRenderizarContactos);
  } catch (error) {
    contactsContainer.innerHTML = "Ocurrió un error";
  }
};

const renderizarContactos = (contactos, contactsContainer) => {
  contactsContainer.innerHTML = "";
